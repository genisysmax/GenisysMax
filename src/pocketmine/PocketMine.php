...
namespace pocketmine {
    ...
    function emit_performance_warnings(ThreadSafeLogger $logger) : void{
        if(PHP_DEBUG !== 0){
...
JIT_WARNING
    ...
    function server(){
        ...
        if(!is_dir(\pocketmine\DATA . 'logs' . DIRECTORY_SEPARATOR)) {
            mkdir(\pocketmine\DATA . 'logs' . DIRECTORY_SEPARATOR);
        }

        $logger = new MainLogger(\pocketmine\DATA . 'logs' . DIRECTORY_SEPARATOR . "server" . date("d-y-m") . ".log", new \DateTimeZone(Timezone::get()));
