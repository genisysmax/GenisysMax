...
namespace pocketmine\network\bedrock\adapter\v388;

use Closure;
use pocketmine\item\Item;
use pocketmine\network\bedrock\adapter\v388\palette\BlockPalette as BlockPalette388;
use pocketmine\network\bedrock\adapter\v388\palette\ItemPalette as ItemPalette388;
use pocketmine\network\bedrock\adapter\v388\protocol as v388;
...
class Protocol388Adapter extends Protocol389Adapter{

	public const PROTOCOL_VERSION = 388; // 1.13.0

	protected const OLD_PACKETS = [
		v388\ProtocolInfo::ACTOR_FALL_PACKET => true,
		v388\ProtocolInfo::UPDATE_BLOCK_PROPERTIES_PACKET => true
	];

	protected const PACKET_MAP = [
		v388\ProtocolInfo::ADD_ITEM_ACTOR_PACKET => v388\AddItemActorPacket::class,
		v388\ProtocolInfo::ADD_PLAYER_PACKET => v388\AddPlayerPacket::class,
		v388\ProtocolInfo::CRAFTING_DATA_PACKET => v388\CraftingDataPacket::class,
		v388\ProtocolInfo::CRAFTING_EVENT_PACKET => v388\CraftingEventPacket::class,
        v388\ProtocolInfo::INVENTORY_CONTENT_PACKET => v388\InventoryContentPacket::class,
        v388\ProtocolInfo::INVENTORY_SLOT_PACKET => v388\InventorySlotPacket::class,
		v388\ProtocolInfo::INVENTORY_TRANSACTION_PACKET => v388\InventoryTransactionPacket::class,
		v388\ProtocolInfo::LOGIN_PACKET => v388\LoginPacket::class,
		v388\ProtocolInfo::MOB_ARMOR_EQUIPMENT_PACKET => v388\MobArmorEquipmentPacket::class,
		v388\ProtocolInfo::MOB_EQUIPMENT_PACKET => v388\MobEquipmentPacket::class,
		v388\ProtocolInfo::START_GAME_PACKET => v388\StartGamePacket::class,
	];

	public function __construct(){
        

        Network::registerPalette(new BlockPaletteEntry(new BlockPalette388()));
		ItemPalette388::lazyInit();

		$this->actorIdentifiers = file_get_contents(\pocketmine\RESOURCE_PATH . "bedrock/v388/entity_identifiers.nbt");
        $this->biomeDefinitions = file_get_contents(\pocketmine\RESOURCE_PATH . "bedrock/v388/biome_definitions.nbt");
	}

    public function getCreativeItems() : array{
        $creativeItemsJson = json_decode(file_get_contents(\pocketmine\RESOURCE_PATH . "bedrock/v388/creativeitems.json"), true);
        $creativeItems = [];
	...
	public function translateBlockId(int $runtimeId) : int{
		...
		return BlockPalette388::getRuntimeFromLegacyId($id, $meta);
	}

	protected function processBlocks(DataPacket $packet, bool $clientToServer) : void{
		if($clientToServer){
			$c = function(int $runtimeId) : int{
				BlockPalette388::getLegacyFromRuntimeId($runtimeId, $id, $meta);
