...
if ($meta >= $item->getMaxDurability()) {
	...
	public function putRecipeIngredient(Item $item) : void{
		if($item->isNull()){
			...
			if($item->hasAnyDamageValue()){
				[$netId, ] = ItemPalette486::getRuntimeFromLegacyId($item->getId(), 0);
				$netData = 0x7fff;
			}else{
				[$netId, $netData] = ItemPalette486::getRuntimeFromLegacyId($item->getId(), $item->getDamage());
			}
			$this->putVarInt($netId);
    ...
    public function writePotionTypes(PotionTypeRecipe $entry):void{
        [$netIdInput, $netDataInput] = ItemPalette486::getRuntimeFromLegacyId($entry->getInputPotionId(), $entry->getInputPotionMeta());
        [$netIdIngredient, $netDataIngredient] = ItemPalette486::getRuntimeFromLegacyId($entry->getIngredientItemId(), $entry->getIngredientItemMeta());
        [$netIdOutput, $netDataOutput] = ItemPalette486::getRuntimeFromLegacyId($entry->getOutputPotionId(), $entry->getOutputPotionMeta());
