...
trait PacketTrait{
	...
	public function getItemStack(\Closure $readExtraCrapInTheMiddle) : Item{
		...
		[$id, $meta] = ItemPalette560::getLegacyFromRuntimeId($netId, $netData);
		...
		return (static function() use ($extraData, $netId, $id, $meta, $cnt) : Item {
			...
			if($netId === ItemPalette560::getRuntimeFromStringId("minecraft:shield")){ //SHIELD
	...
	public function putItemStack(Item $item, \Closure $writeExtraCrapInTheMiddle) : void{
		...
		[$netId, $netData] = ItemPalette560::getRuntimeFromLegacyId($item->getId(),$item instanceof Durable ? 0 : $item->getDamage());
		...
		if($isBlockItem){
			...
			if($block->getId() !== BlockIds::AIR){
				$blockRuntimeId = BlockPalette560::getRuntimeFromLegacyId($block->getId(), $block->getDamage());
			}
		}
    ...
    public function putRecipeIngredient(Item $item) : void{
        if($item->isNull()){
            ...
            if($item->hasAnyDamageValue()){
                [$netId, ] = ItemPalette560::getRuntimeFromLegacyId($item->getId(), 0);
                $netData = 0x7fff;
            }else{
                [$netId, $netData] = ItemPalette560::getRuntimeFromLegacyId($item->getId(), $item->getDamage());
            }
    ...
    public function getRecipeIngredient() : Item{
        ...
        if($descriptorType === ItemDescriptorType::INT_ID_META){
            ...
            $netId = ItemPalette560::getRuntimeFromStringId($this->getString());
            $netData = $this->getLShort();
        }else{
    ...
    public function writePotionTypes(PotionTypeRecipe $entry):void{
        [$netIdInput, $netDataInput] = ItemPalette560::getRuntimeFromLegacyId($entry->getInputPotionId(), $entry->getInputPotionMeta());
        [$netIdIngredient, $netDataIngredient] = ItemPalette560::getRuntimeFromLegacyId($entry->getIngredientItemId(), $entry->getIngredientItemMeta());
        [$netIdOutput, $netDataOutput] = ItemPalette560::getRuntimeFromLegacyId($entry->getOutputPotionId(), $entry->getOutputPotionMeta());
