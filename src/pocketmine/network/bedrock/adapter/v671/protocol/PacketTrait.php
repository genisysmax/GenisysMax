...
trait PacketTrait{
	...
	public function getItemStack(\Closure $readExtraCrapInTheMiddle) : Item
    {
        ...
        [$id, $meta] = ItemPalette671::getLegacyFromRuntimeId($netId, $netData);
        ...
        return (static function () use ($extraData, $netId, $id, $meta, $cnt): Item {
            ...
            if ($netId === ItemPalette671::getRuntimeFromStringId("minecraft:shield")) { //SHIELD
	...
	public function putItemStack(Item $item, \Closure $writeExtraCrapInTheMiddle) : void
    {
        ...
        [$netId, $netData] = ItemPalette671::getRuntimeFromLegacyId($item->getId(), $item instanceof Durable ? 0 : $item->getDamage());
        ...
        if ($isBlockItem) {
            ...
            if ($block->getId() !== BlockIds::AIR) {
                $blockRuntimeId = BlockPalette671::getRuntimeFromLegacyId($block->getId(), $block->getDamage());
            }
        }
    ...
    public function putRecipeIngredient(Item $item) : void{
        if($item->isNull()){
            ...
            if($item->hasAnyDamageValue()){
                [$netId, ] = ItemPalette671::getRuntimeFromLegacyId($item->getId(), 0);
                $netData = 0x7fff;
            }else{
                [$netId, $netData] = ItemPalette671::getRuntimeFromLegacyId($item->getId(), $item->getDamage());
            }
    ...
    public function getRecipeIngredient() : Item{
        ...
        if($descriptorType === ItemDescriptorType::INT_ID_META){
            ...
            $netId = ItemPalette671::getRuntimeFromStringId($this->getString());
            $netData = $this->getLShort();
        }else{
    ...
    public function writePotionTypes(PotionTypeRecipe $entry):void{
        [$netIdInput, $netDataInput] = ItemPalette671::getRuntimeFromLegacyId($entry->getInputPotionId(), $entry->getInputPotionMeta());
        [$netIdIngredient, $netDataIngredient] = ItemPalette671::getRuntimeFromLegacyId($entry->getIngredientItemId(), $entry->getIngredientItemMeta());
        [$netIdOutput, $netDataOutput] = ItemPalette671::getRuntimeFromLegacyId($entry->getOutputPotionId(), $entry->getOutputPotionMeta());
